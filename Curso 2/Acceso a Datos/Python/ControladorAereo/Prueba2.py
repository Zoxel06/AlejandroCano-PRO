# simulador_completo_mejorado.py
import csv
import os
import time
...
def mostrar_vuelos(vuelos: Dict[str, Dict]):
    ...
    for v in sorted(vuelos.values(), key=lambda x: x["id"]):
        print(f"Id:{v['id']} Tipo:{v['tipo']} ETA:{v['eta']} ETD:{v['etd']} Pri:{v['prioridad']} Comb:{v['combustible']} Estado:{v['estado']} t_entrada_cola:{v.get('t_entrada_cola')}")
    print("--------------\n")
...
def _cmp_vuelos(a: Dict, b: Dict, tiempo_simulado: int) -> int:
    ...
    # 2) Aterrizajes con menor combustible (solo si son aterrizajes)
    if a["tipo"] == "ATERRIZAJE" and b["tipo"] != "ATERRIZAJE":
        # a (aterrizaje) preferred if has less combustible than b (b is departure -> treat b's combustible as +inf)
    ...
    rb = retraso(b)
    if ra != rb:
        # mayor retraso primero
        return -1 if ra > rb else 1

    # 4) Desempate alfabético por id (orden ascendente)
    if a["id"] < b["id"]:
        return -1
    if a["id"] > b["id"]:
        return 1
...
def _procesar_minuto(vuelos: Dict[str, Dict], pistas: Dict[str, Dict], tiempo_actual: int):
    ...
    # Recolectar vuelos que se cancelan para liberar posibles pistas inmediatamente
    vuelos_a_cancelar = []
    ...
    for vid in vuelos_a_cancelar:
        ...
        if v["estado"] == "ASIGNADO":
            # buscar la pista que lo tenía asignado
            for p in pistas.values():
                if p.get("vueloAsignadoId") == vid:
                    # liberar pista sin marcar COMPLETADO del vuelo (quedó CANCELADO)
                    p["estadoPista"] = "LIBRE"
                    p["vueloAsignadoId"] = None
                    p["ocupadaHasta"] = None
                    # no incrementamos uso_count aquí: ya se incrementó en asignación
                    break
        v["estado"] = "CANCELADO"
        v["t_completado"] = tiempo_actual
        registrar_evento(f"[t={tiempo_actual}] CANCELADO id_vuelo={vid} motivo=sin_combustible")

    # 2) Liberar pistas cuyo ocupadaHasta <= tiempo_actual
    pistas_liberadas = []
    for p in pistas.values():
    ...
    # 3) Asignaciones automáticas tras liberar pistas
...
def avanzar_tiempo(vuelos: Dict[str, Dict], pistas: Dict[str, Dict], tiempo_actual: int, minutos: int) -> int:
    """
    Avanza 'minutos' minuto a minuto (procesando eventos intermedios).
    Retorna el nuevo tiempo simulado.
    """
    if minutos <= 0:
...
def generar_informe(vuelos: Dict[str, Dict], pistas: Dict[str, Dict], tiempo_simulado: int):
    ...
    # emergencias gestionadas: vuelos que llegaron a prioridad 2 y fueron completados (t_completado not None)
    ...
    with open(LOG_INFORME, "w", encoding="utf-8") as f:
        ...
        # orden por t_completado ascendente
...
def main():
    ...
    while True:
        ...
        print("9) Habilitar/deshabilitar pista (toggle)")
        ...
        if opcion == "1":
            ...
            # toggle habilitada de una pista (útil para pruebas)
            pid = input("Id pista a toggle (ej. R1): ").strip().upper()
            if pid in pistas:
                p = pistas[pid]
                if p["habilitada"] == 1:
                    p["habilitada"] = 0
                    p["estadoPista"] = "DESHABILITADA"
