...
public class UsuarioDTO {
    ...
    public int logearUsuario(String correo, String password){

        // SELECT correo, password FROM usuarios WHERE correo = XXX AND password = XXX
        // resultset.next()

        String query = String.format("SELECT %s FROM %s WHERE %s = ? AND %s = ?",
                SchemaDB.COL_ID,SchemaDB.TAB_USER,SchemaDB.COL_MAIL,SchemaDB.COL_PASS);
