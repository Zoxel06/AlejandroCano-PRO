...
promesa
  ...
  promesa
    .then((res) => {
      ...
      lista.innerHTML = "";
      divAlerta.className = "alert alert-success mb-4";
      divAlerta.textContent = "Se han encontrado " + res.length + " coches";
      res.forEach((element) => {
        let li = document.createElement("li");
        li.innerHTML = `${element.marca} ${element.modelo} ${element.precio}`;
    ...
    .catch((err) => {
      ...
      lista.innerHTML = "";
      lanzarAlerta(err, "warning")
    });
});


  function lanzarAlerta(mensaje, tipo) {
    Swal.fire({
        position: "center",
        icon: tipo,
        text: mensaje,
